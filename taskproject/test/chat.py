...
def get_text():
    input_text = st.text_input(
        "You: ", "Tell me the average of the revenue", key="input")
    return input_text
...
if df.shape[0] > 0:
    agent = create_pandas_dataframe_agent(OpenAI(temperature=0), df, verbose=True,
                                          return_intermediate_steps=True, agent_type=AgentType.ZERO_SHOT_REACT_DESCRIPTION)
    user_input = get_text()
...
if ask_button:
    ...
    with st.spinner('typing...'):
        ...
        answer = json.dumps(response['output'],
